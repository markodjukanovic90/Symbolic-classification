...
\end{equation}
\fxnote{Jano, how to motivate the usage of $\xi^4$. Why exactly the power of 4?}
	...
	\begin{algorithmic}[1]
        ...
        \While{! some of the termination criteria is met }
			...
			\If{$bestNeighbour \neq \emptyset$ and $bestNeighbour.score^{'} < s.best.score{'} * (1 + worsening_{limit})$}
				...
				\If{$bestNeighbour.score^{'} < individual.bestSolution.score{'}$}
...
\end{algorithm}
 ...
 \fxnote{Jano, what is individual here, maybe $s$ instead?It appears also at Line 22. bestScore a Line 15 used but never declared before  } %https://github.com/janoPig/sr_core/tree/main/Hroch
