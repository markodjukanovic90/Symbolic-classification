...
where  $p_{size}>0$ is a strategic parameter and $|s|$ is the size of solution (model) $s$ -- that is the number of nodes of its tree representation. 
 ...
 \emph{Rils-Rols classifier}.  Now, in order to transfer the \textsc{Rils}-\textsc{Rols} algorithms, a state-of-the-art tool for solving the Symbolic regression problems, it has to be turned into a classifier. Here we do it for the problem of (symbolic) binary classification w.r.t.\ a value of treshold parameter. More precisely, for a model $s$ obtained by executing the \textsc{Rils}-\textsc{Rols} algorithm for input data $D$, for $\textbf{x}_i \in D$ the corresponding target value is equal 1 if $s(\textbf{x}_i) \geq treshold$ or 0 otherwise. Interestingly, according to our experimental evaluation, the best \textit{accuracy}  is  obtained by setting $treshold=0.5$. 
